...
@router.put('/update')
async def update_manager(db: Annotated[Session, Depends(get_db)], manager_id: int, update_manager: UpdateManager):
...
async def delete_manager(db: Annotated[Session, Depends(get_db)], manager_id: int):
    manager = db.scalar(select(Manager).where(Manager.id == manager_id))
    if manager is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
